...
function drawValueRight(ctx, value, subtitle, rightX, centerY, accentColor) {
    ctx.textAlign = 'right';
    ...
    if (subtitle) {
        ...
        ctx.fillText(value, rightX, centerY - 9);
        ctx.font = '13px Arial, sans-serif';
        ctx.fillStyle = COLORS.dim;
        ctx.fillText(subtitle, rightX, centerY + 11);
    } else {
        ctx.font = 'bold 18px Arial, sans-serif';
        ctx.fillStyle = accentColor;
        ctx.fillText(value, rightX, centerY);
    }
}

function drawValuePodium(ctx, value, subtitle, x, y, size, accentColor) {
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ...
    if (subtitle) {
        ...
        ctx.fillStyle = COLORS.dim;
        ctx.fillText(`  ${subtitle}`, x + valW, y);
    }
}

async function renderRanking(sorted, page, guild, callerId, options = {}) {
    const {
        title = 'C L A S S E M E N T',
        titleColor = COLORS.gold,
        accentColor = '#57F287',
        footerLabel = 'Classement',
        subtitle = null,
    } = options;
    ...
    const subtitleH = subtitle ? 30 : 0;
    const podiumH = (hasSecond ? 300 : 180) + subtitleH;

    const rest = sorted.slice(3);
    const totalPages = Math.max(1, Math.ceil(rest.length / PER_PAGE));
    const currentPage = Math.min(page, totalPages - 1);
    const pageItems = rest.slice(currentPage * PER_PAGE, (currentPage + 1) * PER_PAGE);
    ...
    if (sorted[0]) {
        ...
        drawValuePodium(ctx, sorted[0].value, sorted[0].subtitle, avX + avatarR + 15, cardY + 65, 18, accentColor);
    }

    if (hasSecond) {
        const { name, avatar } = await fetchMember(guild, sorted[1].userId);
        const cardY = 185 + podiumBaseY;
        const avatarR = 22;
        ...
        drawValuePodium(ctx, sorted[1].value, sorted[1].subtitle, avX + avatarR + 12, cardY + 60, 16, accentColor);
    }
    ...
    if (hasThird) {
        ...
        drawValuePodium(ctx, sorted[2].value, sorted[2].subtitle, avX + avatarR + 12, cardY + 60, 16, accentColor);
    }
    ...
    if (pageItems.length > 0) {
        ...
        for (let i = 0; i < pageItems.length; i++) {
            const s = pageItems[i];
            const rank = currentPage * PER_PAGE + i + 4;
            ...
            drawValueRight(ctx, s.value, s.subtitle, W - 35, centerY, accentColor);
        }
    }
    ...
    if (callerEntry) {
        ...
        drawValueRight(ctx, callerEntry.value, callerEntry.subtitle, W - 35, centerY, accentColor);
    }
    ...
    const footerText = totalPages > 1
        ? `${footerLabel}  •  Page ${currentPage + 1} / ${totalPages}`
        : footerLabel;
...
module.exports = { renderRanking, PER_PAGE };
